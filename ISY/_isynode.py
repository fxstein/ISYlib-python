...
from ISY.IsyExceptionClass import IsyPropertyError, IsyResponseError, IsyRuntimeWarning, \
    IsyCommunicationError, IsyInvalidCmdError, IsySoapError  # , IsyWarning
...
# Node funtions
#
def load_nodes(self, doreload=0):
...
#	  current_node_set = self._nodegroups.viewkeys()
    ...
    if doreload or not hasattr(self, '_nodefolder') or not isinstance(self._nodefolder, dict):
        self._nodefolder = dict()

    if doreload or not hasattr(self, '_folder2addr') or not isinstance(self._folder2addr, dict):
        self._folder2addr = dict()

    if doreload or not hasattr(self, '_name2id') or not isinstance(self._name2id, dict):
        self._name2id = dict()
...
def _gen_member_list(self, doreload=0):
...
def _gen_folder_list(self, nodeinfo, doreload=0):
...
def _gen_nodegroups(self, nodeinfo, doreload=0):
...
def _gen_nodedict(self, nodeinfo, doreload=0):
            ...
            if "name" in idict:
                ...
                if n in self._node2addr:
                    ...
                    warn_mess = "Duplicate Node name \"{0}\":".format(n) \
                                + " \"{1}\"\n\t\"{2}\"".format(n, idict["address"], self._node2addr[n])
                ...
                if not doreload and n in self._name2id:
                    warn_dup_name_list.append((n, idict["address"], self._name2id[n]))
                    warn_mess = "Dup name2id (Node) \"{0}\":".format(n) \
                                + " \"{1}\"\n\t\"{2}\"".format(n, idict["address"], self._name2id[n])
                    warnings.warn(warn_mess, IsyRuntimeWarning)
                else:
...
def load_node_types(self):
    ...
    if typeinfo is None:
        raise IsyCommunicationError("Load Node Type Info Fail: " + self.error_str)
...
def node_enable(self, naddr, enable=True):
    ...
    # self._printXML(resp)
    if resp is None or resp.attrib["succeeded"] != 'true':
        raise IsyResponseError("Node Cmd/Property Set error: node=%s resp=%s " % naddr, resp)
...
def node_set_powerinfo(self, naddr, deviceClass=None,  wattage=None, dcPeriod=None):
    ...
    return self.soapcomm("SetNodePowerInfo", node=node_id, deviceClass=deviceClass, wattage=wattage, dcPeriod=dcPeriod)
...
def node_del(self, naddr):
    ...
    if not node_id:
        raise LookupError("node_del: {0} not a node ( {1}={2} )".format(naddr, node_id, nodetype))
