...
class _IsyNodeBase(IsySubClass):
    ...
    def on(self, val=255):
        ...
        if not str(val).isdigit:
            raise IsyTypeError("On Command: Bad Value: node=%s val=%s" % (self._mydict["address"], str(val)))
    ...
    def _rename(self, cmd, newname):
        ...
        r = self.isy.soapcomm(cmd, id=self._mydict["address"], name=newname)
    ...
    def _set_prop(self, prop, new_value):
        ...
        if not prop in self._setlist:
            if prop == "ST":
                ...
                raise IsyPropertyError("_set_prop: Invalid property Attribute " + prop)
...
class IsyScene(_IsyNodeBase):
    ...
    _setlist = []
    _propalias = {'id': 'address', 'addr': 'address', "group-flag": "flag"}
    ...
    def member_del(self, node):
        r = self.isy.soapcomm("RemoveFromGroup", node=node._get_prop("address"), group=self._mydict["address"])

    def member_add(self, node, flag=16):
        r = self.isy.soapcomm("MoveNode", node=node._get_prop("address"), group=self._mydict["address"], flag=16)
        return r
...
class IsyNodeFolder(_IsyNodeBase):
    ...
    def member_del(self, node):
        """ del Node/Scene or Folder to Folder Obj
            ...
            calls SOAP SetParent()
        """
        r = self.isy.soapcomm("SetParent", node=node._get_prop("address"), nodeType=node.nodeType())
        return r
